...
const CompanyEvaluateCodingTest = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const response = await axios.get(
        `https://campus-hire-backend-457618.an.r.appspot.com/company/get-applications-phase2/${jobId}`,
        {
          headers: {
  ...
  const handleDecision = async (roll_number, decision) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await axios.post("https://campus-hire-backend-457618.an.r.appspot.com/company/evaluate-coding-test", 
        evaluateDetails,
        {
          headers: {
            Authorization: `Bearer ${token}`,
