  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/student/get-job/${jobId}`, {
        ...
        {job && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                <h2 className="font-semibold text-gray-700 mb-2">Job Details</h2>
                <div className="space-y-2">
                  <p><span className="font-medium">Type:</span> {job.type}</p>
                  <p><span className="font-medium">Stipend/Salary:</span> ₹{job.stipend}/month</p>
                  <p><span className="font-medium">Location:</span> {job.location || 'Not specified'}</p>
                </div>
              </div>
              
              <div>
                <h2 className="font-semibold text-gray-700 mb-2">Requirements</h2>
                <div className="space-y-2">
                  {/* <p><span className="font-medium">Skills:</span> {job.required_skills || 'Not specified'}</p> */}
                  <p><span className="font-medium">CGPA:</span> {job.eligibility_cgpa || 'Not specified'}</p>
                </div>
              </div>
            </div>
